...
class CameraManager(object):
    def __init__(self, parent_actor, hud, gamma_correction):
        ...
        self.sensor_front = self._parent.get_world().spawn_actor(
            self.sensors[0][-1],
            carla.Transform(carla.Location(x=2, z=1.5)),
            attach_to=self._parent,
            attachment_type=Attachment.Rigid)
        self.sensor_front_segment = self._parent.get_world().spawn_actor(
            self.sensors[4][-1],
            carla.Transform(carla.Location(x=2, z=1.5)),
            attach_to=self._parent,
            attachment_type=Attachment.Rigid)
    ...
    def set_sensor(self, index, notify=True, force_respawn=False):
        ...
        if needs_respawn:
            ...
            self.sensor_front.listen(
                lambda image: CameraManager._parse_image_rgb(weak_self, image))
            self.sensor_front_segment.listen(
                lambda image: CameraManager._parse_image_segmentation(weak_self, image))
        if notify:
    ...
    @staticmethod
    def _parse_image_rgb(weak_self, image):
        self = weak_self()
        if not self:
            return
        image.save_to_disk('_out/rgb/%08d' % image.frame)

    @staticmethod
    def _parse_image_segmentation(weak_self, image):
        self = weak_self()
        if not self:
            return

        image.save_to_disk('_out/segmentation/%08d' %
                           image.frame)
# ==============================================================================
# -- game_loop() ---------------------------------------------------------------
# ==============================================================================
